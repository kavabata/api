...
module.exports = {
  getLightList: ({ dateStart, dateEnd, zoom, deviceId }, callback = ()=>{} ) => {
  ...
  insertLightByKey: async ({ key, level }) => {
    ...
    return module.exports.insertLightByDeviceId({ deviceId, level });
  },
  insertLightByDeviceId: ({ deviceId, level }) => {
    const getDb = require("./db").getDb;
    const db = getDb();
    const created = new Date();
    const insert = `INSERT INTO light (device_id, level, created) VALUES 
    (${db.escape(deviceId)}, ${db.escape(level)}, ${db.escape(created)})`;
